...
build_binary_tree_times = []
build_type_tree_times = []
#%%
from sklearn.model_selection import train_test_split
from tqdm import tqdm
import time

steps = [0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0]
for step in tqdm(steps, desc='进度'):
    if step == 0:
        train_data_df = dt.train_binary_df
    else:
        train_data_df, X = train_test_split(dt.train_binary_df, test_size=step, random_state=42)
    train_data_df['weight'] = 1
    st = time.perf_counter_ns()
    dt.build_tree(train_data_df)
    ed = time.perf_counter_ns()
    cost_time = ed - st
    build_binary_tree_times.append(cost_time)
#%%
from tqdm import tqdm
import time
...
for step in tqdm(steps, desc='进度'):
    if step == 0:
        train_data_df = dt.train_type_df
    else:
        train_data_df, X = train_test_split(dt.train_type_df, test_size=step, random_state=42)
    train_data_df['weight'] = 1
    st = time.perf_counter_ns()
    dt.build_tree(train_data_df)
    ed = time.perf_counter_ns()
    cost_time = ed - st
    build_type_tree_times.append(cost_time)
#%%
res = [x / 1_000_000_000 for x in build_binary_tree_times]
print('Binary Tree Build Time:')
print(res)
#%%
res = [x / 1_000_000_000 for x in build_type_tree_times]
print('Type Tree Build Time:')
print(res)