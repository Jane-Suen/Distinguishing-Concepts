...
class decisionTree:
    ...
    def execute(self, intersection=True):
        ...
        failed_type_index = self.result_type[self.result_type.apply(lambda row : row['subconcept_set'].isdisjoint(row['tree_subconcept_set']), axis=1)].index
        # 取出决策树预测失败的记录
        self.test_binary_df = self.test_binary_df.loc[failed_type_index]
