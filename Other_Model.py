#%% md
# ## 这个版本：在测试的时候是将binary中失败的记录作为type的测试集
#%%
import numpy as np
import pandas as pd
...
def expand_cartesian(X_df, y):
    ...
    return new_X, new_y
#%%
concept_nums = [164, 165, 166, 168, 170]
...
def execute(concept_num, subconcept_num):
    ...
    for model_name, model in models.items():
        ...
        result_binary_df.to_excel(f'./other_model_result_1/concept_{concept_num}_{subconcept_num}_{model_name}_binary.xlsx', index=False)
        
        res_binary = evaluate_metrics(result_binary)

        flag = False

        if res_binary['acc_zero_indices']:

            flag = True

            X_type_test_data = X_type_test.iloc[res_binary['acc_zero_indices']]
            y_type_test_data = y_type_test[res_binary['acc_zero_indices']]
            ...
            result_type_df.to_excel(f'./other_model_result_1/concept_{concept_num}_{subconcept_num}_{model_name}_type.xlsx', index=False)
        
        import os
        cwd = os.getcwd()
        os.chdir('./other_model_result_1')
...
df.to_excel(f'./other_model_result_1/final_result.xlsx', index=False)
#%%
