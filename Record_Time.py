...
class decisionTree:
    ...
    def execute(self, intersection=True):
        ...
        def decision(df, tree, output_excel=False, excel_filename="result.xlsx"):
                ...
                return expanded_df, record_times
        ...
        self.result_binary, binary_cost_time = decision(self.test_binary_df, self.binary_tree)

        self.result_type, type_cost_time = decision(self.test_type_df, self.type_tree)
        ave_binary_time = sum(binary_cost_time) / 100 / 1_000_000
        max_binary_time = max(binary_cost_time) / 1_000_000
        min_binary_time = min(binary_cost_time) / 1_000_000
        
        
        ave_type_time = sum(type_cost_time) / 100 / 1_000_000
        max_type_time = max(type_cost_time) / 1_000_000
        min_type_time = min(type_cost_time) / 1_000_000
        
        print(f"Binary Decision Tree - Average Time: {ave_binary_time}, Max Time: {max_binary_time}, Min Time: {min_binary_time}")
        
        print(f"Type Decision Tree - Average Time: {ave_type_time}, Max Time: {max_type_time}, Min Time: {min_type_time}")
#%%
